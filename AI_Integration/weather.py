...
class GUI:
    ...
    def setup_axes(self):
       ...
       self.ax[0].set_ylabel("Altitude (m)")
       self.ax[0].grid(True)
   
       self.ax[0].xaxis.set_major_formatter(formatter)
       self.ax[0].yaxis.set_major_formatter(formatter)
       self.ax[0].xaxis.set_major_locator(mticker.MaxNLocator(6))  # fewer ticks
       self.ax[0].tick_params(axis='x', rotation=30)
       ...
       self.ax[1].set_ylabel("Altitude (m)")
       self.ax[1].grid(True)
   
       self.ax[1].xaxis.set_major_formatter(formatter)
       self.ax[1].yaxis.set_major_formatter(formatter)
       self.ax[1].xaxis.set_major_locator(mticker.MaxNLocator(6))
       self.ax[1].tick_params(axis='x', rotation=30)
   
       # =========================
       # GPS LOCATION
       # =========================
       self.ax[2].set_title("GPS Location (Altitude Colored)")
       self.ax[2].set_xlabel("Latitude")
       self.ax[2].set_ylabel("Longitude")
       self.ax[2].grid(True)
   
       self.ax[2].xaxis.set_major_formatter(formatter)
       self.ax[2].yaxis.set_major_formatter(formatter)
       self.ax[2].xaxis.set_major_locator(mticker.MaxNLocator(6))
       self.ax[2].yaxis.set_major_locator(mticker.MaxNLocator(6))
       self.ax[2].tick_params(axis='x', rotation=30)
    ...
    def update(self):
        ...
        if flight_started:
            ...
            # =========================
            # PLOTS 1 & 2 (CONTINUOUS LINES)
            # =========================
            self.ax[0].plot(self.lat, self.alt, color="blue")
            self.ax[1].plot(self.lon, self.alt, color="green")

            # =========================
            # GPS TRAJECTORY (LINE + HEAT)
            # =========================
            if len(self.lat) > 1:

                self.ax[2].plot(self.lat, self.lon, color="purple", linewidth=1)
